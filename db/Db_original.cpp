...
void Db_original::loop(time_t timestamp) {
    ...
    for (map<string, string>::iterator selected = last_mac.begin(); selected != last_mac.end(); ++selected) {
        ...
        Point estimated_point = Triangulation::triangolate(vector_dati, N_schede);
		...
		sqlite3_bind_int(stmt, 6, dato_triang.x );
		sqlite3_bind_int(stmt, 7, dato_triang.y );
...
map<string,int*> Db_original::number_of_rilevations(time_t timestamp_start, time_t timestamp_end) {
    ...
    count_ril.clear(); //pulisco la mappa che e allocata solo una volta e può essere stata usata precedentemente
